...
SELECT categroy, AVG(rating) FROM analytics GROUP BY category ORDER BY avg DESC;
...
SELECT name FROM analytics WHERE rating < 3 AND reviews >1000:
...
SELECT * FROM analytics ORDER BY price DESC LIMIT;
...
SELECT app_name, reviews, min_installs, min_installs / reviews AS proportion FROM analytics WHERE min_installs >= 100000 ORDER BY propoetion DESC LIMIT 1;
